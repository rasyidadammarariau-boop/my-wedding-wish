...
import {
  ...
  CreditCard, Copy, Check, Send, Music, VolumeX, ChevronLeft, ChevronRight, Crown,
} from "lucide-react";
...
const ClassicGoldDemo = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-yellow-50 to-amber-100 text-gray-900">
        ...
        <div className="relative">
          ...
          <section className="py-20 px-4 bg-white">
            <div className="max-w-2xl mx-auto">
              <h2 className="text-3xl font-serif text-amber-800 text-center mb-12">Ucapan & Doa</h2>
              <motion.form initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} onSubmit={handleSubmitComment} className="bg-amber-50 p-6 border-2 border-amber-200 mb-8">
                <div className="space-y-4">
                  <Input value={newComment.name} onChange={(e) => setNewComment({ ...newComment, name: e.target.value })} placeholder="Nama" className="bg-white border-amber-200" />
                  <select value={newComment.presence} onChange={(e) => setNewComment({ ...newComment, presence: e.target.value })} className="w-full px-3 py-2 bg-white border border-amber-200">
                    <option value="hadir">✦ Hadir</option>
                    <option value="tidak hadir">✦ Berhalangan</option>
                  </select>
                  <Textarea value={newComment.message} onChange={(e) => setNewComment({ ...newComment, message: e.target.value })} placeholder="Tulis ucapan..." rows={3} className="bg-white border-amber-200" />
              ...
              <div className="space-y-4">
                {comments.map((c) => (
                  <div key={c.id} className="bg-amber-50 p-4 border border-amber-200">
