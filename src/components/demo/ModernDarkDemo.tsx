...
import {
  ...
  CreditCard, Copy, Check, Send, Music, VolumeX, ChevronLeft, ChevronRight, Sparkles,
} from "lucide-react";
        ...
        <div className="relative">
          ...
          <section className="py-24 px-4 bg-gray-900">
            <div className="max-w-2xl mx-auto">
              <h2 className="text-3xl font-bold text-amber-400 text-center mb-12">Send Wishes</h2>
              <motion.form initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} onSubmit={handleSubmitComment} className="bg-gray-800 p-6 rounded-2xl border border-amber-400/20 mb-8">
                <div className="space-y-4">
                  <Input value={newComment.name} onChange={(e) => setNewComment({ ...newComment, name: e.target.value })} placeholder="Your name" className="bg-gray-900 border-gray-700 text-white" />
                  <select value={newComment.presence} onChange={(e) => setNewComment({ ...newComment, presence: e.target.value })} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-white">
                    <option value="hadir">Attending</option>
                    <option value="tidak hadir">Not Attending</option>
                  </select>
                  <Textarea value={newComment.message} onChange={(e) => setNewComment({ ...newComment, message: e.target.value })} placeholder="Your message..." rows={3} className="bg-gray-900 border-gray-700 text-white" />
              ...
              <div className="space-y-4">
                {comments.map((c) => (
                  <div key={c.id} className="bg-gray-800 p-4 rounded-xl border border-gray-700">
                    <div className="flex justify-between mb-2">
                      <span className="font-semibold text-amber-400">{c.name}</span>
