...
const FloralDreamDemo = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 via-purple-50 to-pink-100 text-gray-800">
      ...
      <div className="fixed top-4 right-4 z-40">
        <Button variant="secondary" size="icon" onClick={() => setIsMuted(!isMuted)} className="bg-white/80 backdrop-blur shadow-lg border-pink-200">
        ...
        <div className="relative">
          ...
          <section className="py-20 px-4 bg-white">
            <div className="max-w-2xl mx-auto">
              <h2 className="text-3xl font-serif text-pink-600 text-center mb-12">💌 Ucapan & Doa</h2>
              <motion.form initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} onSubmit={handleSubmitComment} className="bg-pink-50 p-6 rounded-2xl border border-pink-100 mb-8">
                <div className="space-y-4">
                  <Input value={newComment.name} onChange={(e) => setNewComment({ ...newComment, name: e.target.value })} placeholder="Nama" className="bg-white border-pink-200" />
                  <select value={newComment.presence} onChange={(e) => setNewComment({ ...newComment, presence: e.target.value })} className="w-full px-3 py-2 bg-white border border-pink-200 rounded-md">
                    <option value="hadir">💕 Hadir</option>
                    <option value="tidak hadir">😢 Berhalangan</option>
                  </select>
                  <Textarea value={newComment.message} onChange={(e) => setNewComment({ ...newComment, message: e.target.value })} placeholder="Tulis ucapan..." rows={3} className="bg-white border-pink-200" />
                  <Button type="submit" className="w-full bg-gradient-to-r from-pink-400 to-purple-400 hover:from-pink-500 hover:to-purple-500">
              ...
              <div className="space-y-4">
                {comments.map((c) => (
                  <div key={c.id} className="bg-pink-50 p-4 rounded-xl border border-pink-100">
                    <div className="flex justify-between mb-2">
                      <span className="font-semibold text-pink-600">{c.name}</span>
