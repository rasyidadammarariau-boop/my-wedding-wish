...
import {
  ...
  CreditCard, Copy, Check, Music, VolumeX, ChevronLeft, ChevronRight, Expand,
} from "lucide-react";
          ...
          <section className="py-20 px-4 bg-white">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="relative group cursor-pointer" onClick={() => openLightbox(activeGallery)}>
                ...
                <button onClick={(e) => { e.stopPropagation(); setActiveGallery((prev) => (prev === 0 ? galleryImages.length - 1 : prev - 1)); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur rounded-full shadow-lg border border-pink-200">
                  <ChevronLeft className="h-6 w-6 text-pink-500" />
                </button>
                <button onClick={(e) => { e.stopPropagation(); setActiveGallery((prev) => (prev === galleryImages.length - 1 ? 0 : prev + 1)); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur rounded-full shadow-lg border border-pink-200">
